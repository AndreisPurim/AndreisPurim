...
class ExemploServer(pb2_grpc.MLServerServicer):
    ...
    def ServerFit(self, request, context):
        self.real_data.append(request.dataset)
        self.real_labl.append(request.label)
        self.x_shape = request.x_shape
        self.y_shape = request.y_shape
        answer = {
            'accuracy' : 100,
        }
        print(f"[server.py] Received data")
        return pb2.FitResponse(**answer)
    
    def fitModel(self):
        #Treina o modelo para os dados entradas e labels recebidos
        dataset = numpy.array(self.real_data).reshape(self.x_shape)
        label = numpy.array(self.real_labl).reshape(self.y_shape)
